...
\begin{defn}
  \index{one-to-one}
  \index{injective}
  A function mapping from set $A$ to set $B$ is injective, or one-to-one, if for any two $x$ values in the domain, the function producing equal $y$ values implies that the $x$ values are equal. In mathematical language:
...
\begin{defn}
  \index{onto}
  \index{surjective}
  \(f: A \to B \) is \textbf{surjective} (or \emph{onto}) iff
