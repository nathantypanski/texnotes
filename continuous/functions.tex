...
\begin{defn}[surjective]
  $f: D \to R $ is \definition{surjective}{surjective} or
  \altdefinition{onto}{onto} iff
  \begin{equation}
    \forall y \in R \exists  x \in D \big[f(x)=y\big].
    \label{eq:functions:surjective}
  \end{equation}
\end{defn}

Examples of the surjective property of functions are shown in
\figref{fig:functions:surjective}.
...
\begin{figure}
    \begin{center}
        ...
        \hspace{0.2in}%
        \subfigure[The function $f(x)=x^3$ is surjective because all $y$ values from $(-\infty, \infty)$ have corresponding $x$-values.]
        {\
          \begin{tikzpicture}
            \begin{axis}[
                ylabel={$f(x)=x^3$},
    ...
    \end{center}
    \label{fig:functions:surjective}
  \end{figure}
