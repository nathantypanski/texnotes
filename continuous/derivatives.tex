...
Now we use the point-slope formula (\secref{sec:pointslope}) with $m=3$ to find the equation for our tangent line.
\begin{align*}
  y-y_1 &= m (x-x_1) \\
  y &= mx-mx_1 +y_1 \\
  y&=3x-3\cdot 1 + 1 \\
...
This type of functions is introduced and further explained in \secref{sec:compositefunctions}.
...
Then, we use the point-slope formula (see \secref{sec:pointslope}) to find the equation for a line that approximates our function around that value.
...
\begin{ex}
  ...
  \begin{sol}
    ...
    % Fix the numbering here: first and last eqs should be numbered.
    \begin{align}
      f(x) &= \sqrt{x} = x^{1/2} \\
      ...
      &= \frac{1}{4}
    \end{align}
    % The below sentence should reference the bottom eqn by number.
    This is the slope of $f(x)$ at $x=a$.

    Now, we use this slope in the point-slope formula (\secref{sec:pointslope}) to write our linearized function.
    \begin{align*}
      y-y_1 &= m (x-x_1) \\
      L_a(x)-f(a) &= f'(a) (x-a) \\
...
\end{remark}
The proof of this theorem is found in \secref{proof:lhospital}.
\begin{figure}[h]
  \begin{center}
...
\begin{theorem}[The Derivative Rule for Inverses]\label{th:invderiv}
  ...
  Because a function applied to the inverse of itself should return its own input value, we can start with this relationship.
    ...
    \end{align*}
    % figure out how to lower the period on the last line of the above block.
  \end{proof}
\end{theorem}
...
\begin{ex}
  ...
  \begin{sol}
    ...
    We know from \thref{th:deriv:constantrule}, the constant rule for derivatives, that the derivative of any constant is zero.
