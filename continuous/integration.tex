...
\begin{defn}
  ...
  for every $x$ in $I$, the derivative of $F$ at $x$ is equal to the value
  of $f$ at $x$.
  That is,
  \begin{equation}
...
\begin{ex}
  ...
  \begin{sol}
    ...
    \begin{align*}
      ...
      \int x (x^2+3/2)^9 \ud x &= \frac{x^2+3/2}{20}+C.
    \end{align*}
  \end{sol}
\end{ex}
\begin{ex}
...
Its formula comes from our definition for the derivative product rule, \eref{eq:productrule}:
  \begin{align}
    \lleib{x} \big[ f(x) g(x) \big] &= f'(x)g(x)+f(x)g'(x) \\
    \intertext{If we take the indefinite integral of each side,}
    \int \lleib{x} \big[ f(x) g(x) \big] \ud x &= \int \big[f'(x)g(x)+f(x)g'(x)\big] \ud x \nonumber\\
    \intertext{Now separate out the terms in the righthand side.}
    \int \lleib{x} \big[ f(x) g(x) \big] \ud x &= \int f'(x)g(x)\ud x +\int f(x)g'(x) \ud x \nonumber\\
...
\begin{ex}
  ...
  \[
    ...
    \]
    \begin{sol}
      We use \etref{integration by parts}{equation:intbyparts} with
      \begin{align*}
        u&= x & \ud v &= \cos x \\
        \ud u &= \ud x & v &= \sin x
      \end{align*}
