...
\section{Definition of a Function}
  ...
  if $f$ is a function from $D$ to $R$, we write \[f:D \to R\]
...
\begin{defn}
  \index{one-to-one}
  \index{injective}
  A function mapping from set $D$ to set $R$ is \textbf{injective}, or \textbf{one-to-one}, if for any two $x$ values in the domain, the function producing equal $y$ values implies that the $x$ values are equal. In mathematical language:
