...
like partial fraction decomposition (\secref{sec-integration-pfd}).
...
addition from \secref{sec:alg:assoc:add}.
\begin{equation}
...
As with the commutative law for addition (\secref{sec:alg:comm:add}),
this states that the value of $a * b$ or $b * a$ is independent of the order
in which $a$ and $b$ are taken.\cite[p.~14]{pinter}
\begin{equation}
    a \cdot b = b \cdot a.
