...
proofs}\index{formal proofs}.\footnote{This introduction comes almost entirely from \cite[p.~81]{rosen}. It should be rewritten as soon as possible.}
...
\end{defn}
\begin{defn}
  A \textbf{theorem} is a statement that can be shown to be true, and is associated with a degree of importance.
  \index{theorem}
\end{defn}
\begin{defn}
  A \textbf{lemma} is a ``less important theorem'' that is helpful in the proof of other theorems.
  Interestingly enough, a well-constructed lemma can often be more important than the original proof itself. % from mcsfull.pdf
  \index{lemma}
\end{defn}
\begin{defn}
  A \textbf{corollary} is an additional theorem found only a couple logical steps from its parent theorem.
  \index{corollary}
\end{defn}
\begin{defn}
  A \textbf{conjecture} is a statement that is proposed to be true, but has not yet been proven.
  \index{conjecture}
\end{defn}
