...
\begin{ex}
  \begin{align*}
    \int \underbrace{x}_{u} \underbrace{\cos x \ud x}_{\ud v} &= \underbrace{x}_u \underbrace{\sin x}_v - \int \underbrace{\sin x}_v \underbrace{\ud x}_{\ud u} \\
    &= x \sin x + \cos x +C
  \end{align*}
\end{ex}
...
\begin{ex}
  \[\int\ln x \ud x\]
  \begin{sol} Since $\int \ln x \ud x$ can be written as $\int \ln x \cdot 1 \ud x$, we use the formula $\int u \ud v =  uv - \int v \ud u$ with:
  \begin{align*}
    u&=\ln x & \ud v &= \ud x \\
    \ud u &= \frac{1}{x}\ud x & v&=x
  \end{align*}
  Thus
  \begin{align*}
    \int\ln x \ud x &=
      x \ln x - \int x \cdot \frac{1}{x} \ud x \\
      &= x \ln x - \int \ud x \\
      &= x \ln x - x + C
    \end{align*}
  \end{sol}
...
\begin{ex}[textbook: \#4/pg.441]
  ...
  \begin{align*}
    \int e^{x} \, \cos(x) \ud x
    &= \cos(x) \, e^{x} - \int -\sin(x)e^{x}\ud x \\
    &= e^x \cos x + \underbrace{\int e^x \sin{e^x} \ud x}_{\lets u=e^x} \\
    &=e^x \cos x - \cos{e^x} + C
  \end{align*}
\end{ex}
\begin{ex}
  \[ \int (1-x)^{9}\ud x \]
  This is not an integration by parts problem, it's just a regular $u$-substitution and power rule problem.
  \begin{align*}
    &\lets u = (1-x) & & \lets \ud u = -1\ud x
  \end{align*}
  $$ \frac{-(1-x)^{10}}{10}+C $$
\end{ex}
\begin{ex}
  \[ \int 3x^{2} \, \cos{x^{3}} \ud x \]
  Again, this is not an integration by parts problem, because we can spot the derivative of $x^3$ clearly in the integrand:
  \begin{align*}
    &\lets u=x^{3} & &\lets du = 3x^{2}\ud x
  \end{align*}
  \begin{align*}
    \int 3x^2 \, \cos{x^{3}} \ud x &=
    \int \cos u \ud u \\
...
\begin{ex}
  ...
  \end{align*}
  \begin{align*}
    \int x \, \cos x \ud x &=x \sin(x) - \int \sin x \ud x+C \\
    &=x \sin x+\cos x+C
  \end{align*}
\end{ex}
\begin{ex}
...
\begin{ex}
  ...
  \begin{align*}
    \int x \ln x \ud x &=
    \ln x \frac{x^{2}}{2} - \int \frac{1}{x} \cdot \frac{x^{2}}{2} \ud x \\
    &= \ln x \frac{x^2}{2} - \frac {x^2}{4} + C
  \end{align*}
\end{ex}
\begin{ex}
  \[ \int x e^{x^2} \ud x \]
  Not a chain rule problem. Just let $u=x^2$.
  \[ \int x e^{x^2} \ud x = \frac{1}{2} e^{x^2}+C \]
\end{ex}
\begin{ex}
  \[ \int x^3 e^{x^2} \ud x \]
...
\end{ex}
\begin{ex}
  Dr. Martin called this one ``strange.''
  \[ \int \ln x \ud x \]
  \begin{align*}
  u &= \ln x & \ud v &=\ud x \\
  \ud u &= \frac {1}{x} \ud x & v &= x
  \end{align*}
  \begin{align*}
    \int \ln x \ud x &=\ln x - \int \frac{1}{x} x \ud x\\
    &=\ln x - x + C
  \end{align*}
\end{ex}
\begin{ex}
...
You'll want to understand these trig identities:

\[ 1-sin^{2}\theta=cos^{2}\theta \]
\[ 1+tan^{2}\theta=sec^{2}\theta \]

Useful for problems that look this:
...
\begin{itemize}
  \item $ \sqrt{x^{2}-a^{2}} \implies
    \lets x=a\sec \theta \text{,} \qquad
    \ud x = a \sec \theta \, \tan \theta \ud \theta $
  \item $ \sqrt{a^{2}-x^{2}} \implies
    \lets x = a \sin \theta \text{,} \qquad
    \ud x = a \cos \theta \ud \theta $
  \item $ \sqrt{x^2+a^2} \implies
    \lets x = a \tan \theta \text{,} \qquad
    \ud x = \sec^2 \theta \ud \theta$
\end{itemize}
