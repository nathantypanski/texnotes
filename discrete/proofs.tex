...
\definition{conjecture}{conjecture}.

A \definition{fact}{fact} or \definition{result}{result} is the simplest type of statement we might
prove. Sometimes proofs are required, but more often they are simply
demonstrated, implied, or obvious.

\definition{Theorems}{theorem} are where proofs start to become important. They are the
foundations of complex mathematical logic. Some theorems are
less important than others, and we call these \definition{lemmata}{lemmata}.
A \definition{lemma}{lemma} is a less important theorem, often used in proving
our main theorem. After our theorem is proven, we can often draw
\definition{corollaries}{corollary}, theorems that follow
easily from our proven theorem.

Some proofs are \definition{formal proofs}{proofs!formal}, which use the rules of inference to
...
Thus, we most often will encounter \definition{informal proofs}{proofs!informal}.
These proofs establish truth using language and logic that makes sense to humans.

\section{Direct Proof}

A \definition{direct proof}{proof!direct} starts with known facts, and the statement we are trying
...
An \definition{indirect proof}{proof!indirect} is, basically, any proof that is not a \emph{direct proof}.
...
The first method of indirect proof we will discuss is the \definition{proof by contrapositive}{proof!by contrapositive}.
It is often more useful to work with the contrapositive something than the original statement.

Remembering our definition of the contrapositive from \secref{sec:propintro},
let's cover some examples.

\begin{defn}
  The \definition{contrapositive}{contrapositive} of the statement $P \to Q$
  is the statement $\neg Q \to \neg P$.
...
\begin{ex}
  ...
  \begin{proof}
    We will use proof by contraposition.\footnote{Same thing as proof by
      contrapositive (\defnref{defn:contrapositive}).}
    \begin{quote}
...
To use \definition{proof by contradiction}{proof!by contradiction} to prove a proposition $P$,
