...
\begin{figure}[h]
  ...
  \end{center}
  \caption{A plot of \eref{eq:firstlimit}. Note the hole at \(x=1\).}
\end{figure}
...
For example, $h(x)=\sin \frac{1}{x}$ oscillates indefinitely as \(x \to 0\), as shown in \figref{fig:p1sin1x}.
\begin{figure}[H]
  \begin{center}
...
\figref{fig:p1sin1x}. We can generalize this kind of behavior as a theorem, the
...
\section{\emph{Epsilon-Delta} Definition of a Limit}\index{limit!epsilon-delta definition}
...
\begin{ex}
  ...
  \[ \lim_{x \to 4} \frac{4x-x^2}{2-\sqrt x} \text{.} \]
    ...
    \end{figure}
  \begin{sol}
    First we multiply by the conjugate of the denominator to remove the radical from it. For more detail on conjugates, see \secref{app:def:conjugate}.
    \begin{align*}
...
\begin{ex}
  ...
  Using \thref{th:sandwich},\footnote{How?} we can see that this limit approaches $1$.
\end{ex}
...
\begin{figure}[h]
    ...
    \caption{A plot of \eref{eq:indeterminate_form}.}
