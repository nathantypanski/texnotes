...
% packages %
%%%%%%%%%%%%
\useprogram[letterpaper, twoside]{geometry}  % set the margins to 1in on all sides
\useprogram[bookmarks, hidelinks]{hyperref} % generates pdf index
\useprogram{cclicenses}
\useprogram{yfonts} % fancy first letters
\useprogram{graphicx}              % to include figures
\useprogram{mathtools}             % great math stuff
\useprogram{amsfonts}              % for blackboard bold, etc
\useprogram{amsthm}                % better theorem environments
\useprogram{thmtools}              % better interaction with theorem environments
\useprogram{bigints}               % big integrals
\useprogram{color}                 % color
\useprogram[usenames,dvipsnames]{xcolor} % more color
\useprogram{verbatim}              % multiline comments
\useprogram{array}                 % better table formatting
\useprogram{microtype}             % sexier typesetting
\useprogram{url}                   % urls
\useprogram{pgfplots}              % sexier graphs
\useprogram{fancyhdr}              % being fancy
\useprogram{float}                 % more figure control
\useprogram{wrapfig}               % wrapping text around figures
\useprogram{subfigure}             % side-by-side figures
\useprogram{polynom}               % polynomial long division
\useprogram{setspace}              % used to set line spacing, e.g  \setstretch{1.3}
\useprogram{makeidx}               % indexing
\useprogram[square, numbers, comma, sort&compress]{natbib}  % Use the ``Natbib'' style for the references in the Bibliography
\useprogram{caption3}              % load caption package kernel first
\tionOption{parskip} % disable ``parskip'' caption option
\[font=smaller]{caption}
\[europeancurrents, europeanvoltages, americanresistors, cuteinductors, americanports, nosiunitx, noarrowmos]{circuitikz}            % logic gates
\{tikz} % stuff for circuits and more
